...
def lstm_model(sequences_length_for_training, embedding_dim, embedding_matrix, vocab_size):
    ...
    context_embedder = TimeDistributed(
        Embedding(vocab_size + 1, GLOVE_EMBEDDING_DIM, weights=[embedding_matrix], input_length=embedding_dim,
    ...
    context_encoder_intermediate1 = Bidirectional(
        LSTM(600, input_shape=(ONE_SIDE_CONTEXT_SIZE, CONV_DIM), consume_less='gpu', dropout_W=0.3, dropout_U=0.3,
    ...
    # encode_left = Attention(name='encode-left-attention')(
    # context_encoder(context_encoder_intermediate1(convoluted_left)))
    # encode_right = Attention(name='encode-right-attention')(
    # context_encoder(context_encoder_intermediate1(convoluted_right)))
...
def batch_gen_consecutive_context_segments_from_big_seq(typeOfBatch, X_with_doc, Y_with_doc, batch_size,
        ...
        for i in range(total_seq):  # sample count
            ...
            if typeOfBatch == "train":  # Skip documents only when they are for training, for testing test on all the documents
                if total_seq < 2 * one_side_context_size + 1:
                    print "Too Small sequence: Found %d, required %d. DROPPING DOCUMENT" % (
                        total_seq, 2 * one_side_context_size + 1)
                    break
            ...
            if xL_i < 0:
                ...
                X_temp_Left, Y_temp_Left = np.concatenate((np.zeros(shpX), X_temp_Left), axis=0), np.concatenate(
                    (np.zeros(shpY), Y_temp_Left), axis=0)
                # print X_temp_Left.shape
                # print X_temp_Left
                # print "-----------------"
            else:
            ...
            if xR_j > total_seq:
                X_temp_Right, Y_temp_Right = np.array(X[xR_i: total_seq]), np.array(
                    Y[xR_i: total_seq])  # Create resizable array not view
                # print "+++++++++++++++++++++++++++"
                ...
                X_temp_Right, Y_temp_Right = np.concatenate((X_temp_Right, np.zeros(shpX)), axis=0), np.concatenate(
                    (Y_temp_Right, np.zeros(shpY)), axis=0)
                # print X_temp_Right
                # print X_temp_Right.shape
                # print "==========================="
            else:
            ...
            X_Mid_batch.append(X[sent_i].reshape((1, -1))), Y_Mid_batch.append(
                Y[sent_i])  # X's should be TimeDistributed but not Y's
...
def custom_fit(X_train, Y_train, X_test, Y_test, model, batch_size, epochs=10):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_count, (batch_X_left, batch_X_mid, batch_X_right, batch_Y_mid) in enumerate(
                batch_gen_consecutive_context_segments_from_big_seq("train", X_train, Y_train, batch_size,
                                                                    ONE_SIDE_CONTEXT_SIZE)):
            ...
            try:
                ...
                progbar.prog_bar(True, total_sentences, epochs, batch_size, epoch, batch_count, speed=speed,
                                 data={'Loss': tr_loss, 'Acc': tr_acc, 'Rec': tr_rec})
        ...
        for batch_X_left, batch_X_mid, batch_X_right, batch_Y_mid in batch_gen_consecutive_context_segments_from_big_seq(
                "test", X_test, Y_test, batch_size, ONE_SIDE_CONTEXT_SIZE):
            # batch_Y_mid = to_categorical(batch_Y_mid, nb_classes=2)
...
def save_predictions(type_of_data, X_test, Y_test, model, batch_size, summary_only=False):
    ...
    for i, (Xi_test, Yi_test) in enumerate(zipped):
        ...
        print Xi_test.shape
        pred_per_doc = []
        Xi_test, Yi_test = Xi_test.reshape((1,) + Xi_test.shape), Yi_test.reshape(
            (1,) + Yi_test.shape)  # Convert to format of 1 document
        for batch_X_left, batch_X_mid, batch_X_right, batch_Y_mid in batch_gen_consecutive_context_segments_from_big_seq(
                "test", Xi_test, Yi_test, batch_size, ONE_SIDE_CONTEXT_SIZE):
        ...
        actual_avg_seg_length, result = helper.windiff_and_pk_metric_ONE_SEQUENCE(Yi_test[0], pred_per_doc,
                                                                                  window_size=-1, rounded=False,
...
def testing_on_data(type_of_data, X_test, Y_test, model, batch_size, summary_only=False, visualize=False):
    ...
    print "AVG segment length in test data: %f, std: %f" % (
        np.mean(avg_segment_lengths_across_test_data), np.std(avg_segment_lengths_across_test_data))
    print "WinDiff metric (Windiff_r):: avg: %f | std: %f | min: %f | max: %f" % (
        np.mean(data['wd_r']), np.std(data['wd_r']), np.min(data['wd_r']), np.max(data['wd_r']))
    ...
    print "Pk metric:: avg: %f | std: %f | min: %f | max: %f" % (
        np.mean(data['pk']), np.std(data['pk']), np.min(data['pk']), np.max(data['pk']))
    print('___________________________________')
...
if __name__ == "__main__":
    ...
    # For which_model == 2
    SAMPLE_TYPE_wiki, X_wiki, Y_wiki, trained_sample_handler = get_input(sample_type=4, shuffle_documents=True,
                                                                         pad=False)
    NO_OF_SAMPLES, MAX_SEQUENCE_LENGTH, EMBEDDING_DIM = X_wiki.shape[0], -1, X_wiki[0].shape[
        1]  # MAX_SEQUENCE_LENGTH is is already padded
    ...
    train_LSTM(X_wiki, Y_wiki, model, embedding_W, train_split=0.8, epochs=30, batch_size=200)

    batch_size = 200

    # For which_model == 2
    # Biography data for training
    SAMPLE_TYPE_bio, X_bio, Y_bio, trained_sample_handler = get_input(sample_type=5, shuffle_documents=False, pad=False,
                                                                      trained_sent2vec_model=trained_sample_handler)
    NO_OF_SAMPLES, MAX_SEQUENCE_LENGTH, EMBEDDING_DIM = X_bio.shape[0], -1, X_bio[0].shape[
        1]  # MAX_SEQUENCE_LENGTH is is already padded

    testing_on_data("Biography", X_bio, Y_bio, model, batch_size, summary_only=True, visualize=False)

    # Clinical, Fiction, Wikipedia - Only for testing
    SAMPLE_TYPE_cli, X_cli, Y_cli, trained_sample_handler = get_input(sample_type=4, shuffle_documents=False, pad=False,
                                                                      trained_sent2vec_model=trained_sample_handler)
    testing_on_data("Clinical", X_cli, Y_cli, model, batch_size, summary_only=True, visualize=False)

    SAMPLE_TYPE_fic, X_fic, Y_fic, trained_sample_handler = get_input(sample_type=6, shuffle_documents=False, pad=False,
                                                                      trained_sent2vec_model=trained_sample_handler)

    testing_on_data("Fiction", X_fic, Y_fic, model, batch_size, summary_only=True, visualize=False)

    SAMPLE_TYPE_wikitest, X_wikitest, Y_wikitest, trained_sample_handler = get_input(sample_type=7,
                                                                                     shuffle_documents=False, pad=False,
                                                                                     trained_sent2vec_model=trained_sample_handler)

    testing_on_data("Wikipedia(DEVELOPMENT)", X_wikitest, Y_wikitest, model, batch_size, summary_only=True,
                    visualize=False)
    ...
    pdb.set_trace()
    # custom_fit(X_wiki, Y_wiki, X_cli, Y_cli, model, batch_size=40, epochs=30)
