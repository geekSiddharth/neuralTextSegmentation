...
def get_input(sample_type, shuffle_documents, pad, trained_sent2vec_model=None):
    ...
    if sample_type == 1:
        ...
        sample_type, samples = data_handler.get_samples()  # Get samples, each sample is a set of INPUT_VECTOR_LENGTH
        #  consecutive sentences. No document information captured
    elif sample_type in (2, 3):
        # type2 : Get samples, each sample is a document (a set of sentences resulting in a sequence), or,
        # (NUM_DOCUMENTS, NUM_SENTENCES, SENTENCE) type3 : Same as type2 just merge the samples to remove the
        # sequence information and treat as simple sentence classification problem, i.e. (TOTAL_NUM_SENTENCES,
        # SENTENCE) This processing will be done in the cnn_clssifier.py itself.
        sample_type, samples = data_handler.get_sequence_samples()
        # sample_type, samples = data_handler.get_sequence_samples_PARALLEL()  # Get samples, each sample is a
        # document (a set of sentences resulting in a sequence)
    elif sample_type == 4:
    ...
    for _idx, sample in enumerate(samples):
        ...
        if CREATE_WIKI_TEST_SET:
            ...
            # Traditional code
            if not _idx % 50:
                progbar.simple_update("Converting doc to martices", _idx + 1, _total_samples,
    ...
    if pad:
        ...
        print
        "Padding sequences. Doc-lengths: Mean=%d, Std=%d" % (np.mean(doc_lengths), np.std(doc_lengths))
...
#     main_data = get_input(sample_type=2, shuffle_documents=True, pad=False)
#
#     with open('main_train_data', 'w') as f:
#         cPickle.dump(main_data, f, protocol=cPickle.)
#
#     with open('main_train_data', 'rb') as f:
#         cPickle.load(f)
#     pdb.set_trace()
