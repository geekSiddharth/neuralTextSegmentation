...
import helper
import numpy as np
from keras.layers import Dense, Dropout, Embedding, Flatten, RepeatVector, Input, Merge, Convolution1D, LSTM, \
    Bidirectional
...
def lstm_model(sequences_length_for_training, embedding_dim, embedding_matrix, vocab_size):
    ...
    encoded = Bidirectional(
        LSTM(512, input_shape=(SEQUENCES_LENGTH_FOR_TRAINING, CONV_DIM), return_sequences=True, stateful=False),
...
def custom_fit(X, Y, model, batch_size, train_split=0.8, epochs=10):
    if train_split == 0:
        ...
        for epoch in range(epochs):
            ...
            for batch_X, batch_Y, _ct in batch_gen_SHORT_SEQ_for_training_from_big_seq(X_train, Y_train, batch_size,
                                                                                       SEQUENCES_LENGTH_FOR_TRAINING):
                # pdb.set_trace()
    ...
    for batch_X, batch_Y, _ct in batch_gen_SHORT_SEQ_for_training_from_big_seq(X_test, Y_test, batch_size,
                                                                               SEQUENCES_LENGTH_FOR_TRAINING):
        # batch_Y_vec = to_categorical_MULTI_DIM(batch_Y, nb_classes=2)
    ...
    print("Predicting...")
    predictions = []
    for batch_X, batch_Y, _sent_count in batch_gen_SHORT_SEQ_for_training_from_big_seq(X_test, Y_test, batch_size,
                                                                                       SEQUENCES_LENGTH_FOR_TRAINING):
...
def train_LSTM(X, Y, model, embedding_W, train_split=0.8, epochs=10, batch_size=32):
    ...
    if which_model == 2:
        ...
        X_train, Y_train, X_test, Y_test = split_data(X, Y, train_split=train_split)
        model.fit(X_train, Y_train, shuffle=False, nb_epoch=epochs, batch_size=batch_size,
        ...
        print
        "NOTE: Truncating the Test dataset(clinical) from %d sentences to %d sentences." % (
...
if __name__ == "__main__":
    ...
    # SAMPLE_TYPE_cli, X_cli, Y_cli, trained_sample_handler = get_input(sample_type=4, shuffle_documents=False, pad=True,
    #                                                                   trained_sent2vec_model=trained_sample_handler)
